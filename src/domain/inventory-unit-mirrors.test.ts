...
import {
  ...
  mirrorsOf,
  padWidthFor,
  UNIT_DELETE_BLOCKED_MESSAGE,
  canHardDeleteUnit,
  nextStartFor,
  unitDeleteBlockedReason,
  planBulkGeneration,
  withUnitRemoved,
...
import type {
  ConditionCounts, InventoryItem, InventoryUnit, UnitCounts,
...
describe('nextStartFor', () => {
  it('carries on from the highest number already used', () => {
    expect(nextStartFor('MIC', ['MIC-001', 'MIC-002', 'MIC-003'])).toBe(4)
  })

  it('ignores the padding, which is a width and not a value', () => {
    // `padWidthFor` decides the new run's width from its own size, so reading
    // the old width back would be a second, disagreeing answer.
    expect(nextStartFor('LX', ['LX-017'])).toBe(18)
    expect(nextStartFor('LX', ['LX-17'])).toBe(18)
  })

  it('starts at one when nothing uses the prefix yet', () => {
    expect(nextStartFor('NEW', [])).toBe(1)
    expect(nextStartFor('NEW', ['MIC-001', 'LX-004'])).toBe(1)
  })

  it('treats a trailing hyphen as the same prefix', () => {
    // The format supplies the separator, so `MIC` and `MIC-` are one run and
    // must not be numbered independently.
    expect(nextStartFor('MIC-', ['MIC-007'])).toBe(8)
    expect(nextStartFor('MIC', ['MIC-007'])).toBe(8)
  })

  it('does not claim a longer prefix that merely starts the same way', () => {
    // The failure this exists for: `MIC` adopting `MIC-STAND-050` and
    // suggesting 51, colliding with nothing and skipping the real next number.
    expect(nextStartFor('MIC', ['MIC-001', 'MIC-STAND-050'])).toBe(2)
    expect(nextStartFor('MIC-STAND', ['MIC-001', 'MIC-STAND-050'])).toBe(51)
  })

  it('ignores codes whose suffix is not a plain number', () => {
    // A hand-typed `MIC-SPARE` must not be parsed into anything.
    expect(nextStartFor('MIC', ['MIC-SPARE', 'MIC-2B', 'MIC-004'])).toBe(5)
    expect(nextStartFor('MIC', ['MIC-SPARE'])).toBe(1)
  })

  it('takes the highest rather than the last or the count', () => {
    // Codes arrive in whatever order the query returned them, and a deleted
    // unit leaves a gap that must not renumber the survivors.
    expect(nextStartFor('MIC', ['MIC-009', 'MIC-002', 'MIC-005'])).toBe(10)
    expect(nextStartFor('MIC', ['MIC-001', 'MIC-050'])).toBe(51)
  })

  it('numbers a bare run when there is no prefix at all', () => {
    // `generateAssetCodes` emits the number by itself in that case.
    expect(nextStartFor('', ['004', '011'])).toBe(12)
    expect(nextStartFor('   ', ['004'])).toBe(5)
    expect(nextStartFor('', ['MIC-900'])).toBe(1)
  })

  it('matches the prefix regardless of case, as duplicate detection does', () => {
    // `planBulkGeneration` compares codes lowercased, so a suggestion that
    // ignored case would hand back a number it then reports as taken.
    expect(nextStartFor('mic', ['MIC-003'])).toBe(4)
  })

  it('is not confused by a prefix containing regular-expression characters', () => {
    expect(nextStartFor('A.B', ['A.B-002', 'AXB-009'])).toBe(3)
    expect(nextStartFor('C+D', ['C+D-004'])).toBe(5)
  })

  it('tolerates surrounding whitespace on a stored code', () => {
    expect(nextStartFor('MIC', [' MIC-006 '])).toBe(7)
  })

  it('produces a start that generates codes nothing already owns', () => {
    // The property that matters, checked end to end against the generator.
    const existing = ['MIC-001', 'MIC-002', 'MIC-003']
    const plan = planBulkGeneration({
      prefix: 'MIC',
      start: nextStartFor('MIC', existing),
      count: 3,
      existingCodes: existing,
    })

    expect(plan.valid).toBe(true)
    expect(plan.valid && plan.duplicates).toEqual([])
    expect(plan.valid && plan.codes).toEqual(['MIC-004', 'MIC-005', 'MIC-006'])
  })
})

describe('canHardDeleteUnit', () => {
  const clean = {
    unit_id: 'u-1',
    status: 'available',
    condition: 'good',
  ...
  it('clears a unit nothing has happened to', () => {
    // The case the feature exists for: generated by mistake, moments ago.
    expect(canHardDeleteUnit(clean)).toBe(true)
  })

  it('refuses a unit that has ever changed status', () => {
    // The load-bearing check. Rules require a status change to set this, so its
    // absence is what proves no asset event names the unit.
    expect(canHardDeleteUnit({ ...clean, last_lifecycle_event_id: 'e-1' } as InventoryUnit))
      .toBe(false)
  })

  it('refuses a unit that has been to maintenance', () => {
    expect(canHardDeleteUnit({ ...clean, maintenance_record_ids: ['m-1'] } as InventoryUnit))
      .toBe(false)
    expect(canHardDeleteUnit({ ...clean, current_maintenance_record_id: 'm-1' } as InventoryUnit))
      .toBe(false)
  })

  it('refuses a unit with a repair planned for it', () => {
    // Checked on its own: a plan may be attached without the status moving, so
    // the lifecycle pointer would still be absent.
    expect(canHardDeleteUnit({ ...clean, planned_maintenance_record_id: 'm-1' } as InventoryUnit))
      .toBe(false)
  })

  it('refuses a unit that is not simply sitting on the shelf', () => {
    for (const status of ['in_use', 'in_maintenance', 'lost', 'retired'] as const) {
      expect(canHardDeleteUnit({ ...clean, status } as InventoryUnit), status).toBe(false)
    }
  })

  it('refuses a unit somebody is holding', () => {
    expect(canHardDeleteUnit({ ...clean, using_team_id: 't-1' } as InventoryUnit)).toBe(false)
    expect(canHardDeleteUnit({ ...clean, using_member_uid: 'u-9' } as InventoryUnit)).toBe(false)
    expect(canHardDeleteUnit({ ...clean, checked_out_at: 'ts' } as unknown as InventoryUnit))
      .toBe(false)
  })

  it('is not fooled by an empty history array', () => {
    // Absent and empty mean the same thing and must answer the same way.
    expect(canHardDeleteUnit({ ...clean, maintenance_record_ids: [] } as InventoryUnit)).toBe(true)
  })

  it('says what to do instead', () => {
    expect(UNIT_DELETE_BLOCKED_MESSAGE).toContain('Retire it instead')
  })
})

describe('withUnitRemoved', () => {
  const start = withUnitsAdded(EMPTY_MIRRORS, [
    { status: 'available', condition: 'good' },
    { status: 'available', condition: 'fair' },
    { status: 'available', condition: 'good' },
  ])

  it('undoes exactly what adding the unit did', () => {
    // The property the parent's counts depend on.
    const back = withUnitRemoved(start, { status: 'available', condition: 'good' })
    const rebuilt = withUnitsAdded(EMPTY_MIRRORS, [
      { status: 'available', condition: 'good' },
      { status: 'available', condition: 'fair' },
    ])

    expect(back).toEqual(rebuilt)
  })

  it('drops the active total by one, which is what Rules check', () => {
    const back = withUnitRemoved(start, { status: 'available', condition: 'good' })

    expect(back.unit_counts.active_total).toBe(start.unit_counts.active_total - 1)
    expect(back.unit_counts.available).toBe(start.unit_counts.available - 1)
    expect(back.condition_counts.good).toBe(start.condition_counts.good - 1)
  })

  it('leaves the parent internally consistent', () => {
    const back = withUnitRemoved(start, { status: 'available', condition: 'fair' })
    const { available, unusable_on_hand, in_use, in_maintenance, lost } = back.unit_counts

    expect(back.unit_counts.active_total)
      .toBe(available + unusable_on_hand + in_use + in_maintenance + lost)
  })

  it('empties back to nothing rather than going negative', () => {
    const one = withUnitsAdded(EMPTY_MIRRORS, [{ status: 'available', condition: 'good' }])
    const none = withUnitRemoved(one, { status: 'available', condition: 'good' })

    expect(none).toEqual(EMPTY_MIRRORS)
  })
})

describe('unitDeleteBlockedReason', () => {
  const clean = {
    unit_id: 'u-1',
    asset_code: 'MIC-001',
    status: 'available',
  ...
  it('says nothing about a unit that can be deleted', () => {
    expect(unitDeleteBlockedReason(clean)).toBeNull()
  })

  it('agrees with canHardDeleteUnit on every case', () => {
    // The property that matters. This function exists to explain the decision,
    // never to make a different one — a reason that came back null where
    // `canHardDeleteUnit` says false would enable the button on a unit the
    // service and Rules both refuse.
    const cases: InventoryUnit[] = [
      clean,
      { ...clean, last_lifecycle_event_id: 'e-1' } as InventoryUnit,
      { ...clean, maintenance_record_ids: ['m-1'] } as InventoryUnit,
      { ...clean, maintenance_record_ids: [] } as InventoryUnit,
      { ...clean, current_maintenance_record_id: 'm-1' } as InventoryUnit,
      { ...clean, planned_maintenance_record_id: 'm-1' } as InventoryUnit,
      { ...clean, using_team_id: 't-1' } as InventoryUnit,
      { ...clean, using_member_uid: 'u-9' } as InventoryUnit,
      { ...clean, checked_out_at: 'ts' } as unknown as InventoryUnit,
      { ...clean, retirement_reason: 'disposed' } as unknown as InventoryUnit,
      ...(['in_use', 'in_maintenance', 'lost', 'retired'] as const).map(
        (status) => ({ ...clean, status }) as InventoryUnit,
      ),
    ]

    for (const unit of cases) {
      expect(unitDeleteBlockedReason(unit) === null, JSON.stringify(unit))
        .toBe(canHardDeleteUnit(unit))
    }
  })

  it('uses the linked-records wording only when records are what is linked', () => {
    // An available unit with a pointer attached. This is the one case the
    // requested message actually describes.
    for (const extra of [
      { last_lifecycle_event_id: 'e-1' },
      { maintenance_record_ids: ['m-1'] },
      { current_maintenance_record_id: 'm-1' },
      { planned_maintenance_record_id: 'm-1' },
    ]) {
      expect(unitDeleteBlockedReason({ ...clean, ...extra } as InventoryUnit))
        .toBe(UNIT_DELETE_BLOCKED_MESSAGE)
    }
  })

  it('does not blame linked records for a unit that is simply in use', () => {
    // Saying "this unit has linked records" about equipment somebody is merely
    // holding is a false explanation, which is worse than a vague one.
    const reason = unitDeleteBlockedReason({ ...clean, status: 'in_use' } as InventoryUnit)

    expect(reason).not.toBe(UNIT_DELETE_BLOCKED_MESSAGE)
    expect(reason).toContain('checked out')
  })

  it('does not tell an already-retired unit to be retired', () => {
    const reason = unitDeleteBlockedReason({ ...clean, status: 'retired' } as InventoryUnit)

    expect(reason).toContain('already retired')
    expect(reason).not.toContain('Retire it instead')
  })

  it('only suggests retiring where the unit page offers it', () => {
    // `OFFERED_ACTIONS` allows retiring from available and lost, and not from
    // in_use or in_maintenance. Pointing at an action that is not on screen
    // would send somebody looking for a button that does not exist.
    for (const status of ['in_use', 'in_maintenance'] as const) {
      expect(unitDeleteBlockedReason({ ...clean, status } as InventoryUnit), status)
        .not.toContain('Retire it instead')
    }
    expect(unitDeleteBlockedReason({ ...clean, status: 'lost' } as InventoryUnit))
      .toContain('Retire it instead')
  })

  it('gives every blocked unit something to read', () => {
    for (const status of ['in_use', 'in_maintenance', 'lost', 'retired'] as const) {
      const reason = unitDeleteBlockedReason({ ...clean, status } as InventoryUnit)
      expect(reason, status).toBeTruthy()
      expect((reason ?? '').length, status).toBeGreaterThan(20)
    }
  })
})
