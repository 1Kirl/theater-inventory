...
import type { DeepLinkOutcome } from '@/features/inventory/record-deep-link'
import { paths } from '@/routes/paths'

/**
 * What a scanned label shows when it does not open straight onto the record.
 *
 * One component for every label type. Units had this first; items got labels
 * later and use the same thing rather than a second organization-switch
 * mechanism that could drift from it. Only the noun changes.
 *
 ...
 */
export interface DeepLinkWording {
  /** Lowercase, mid-sentence. "equipment", "inventory item". */
  noun: string
  /** Heading on the switch card. */
  belongsTo: string
  /** The switch button. */
  switchAction: string
}
...
export function DeepLinkNotice({
  ...
  outcome: DeepLinkOutcome<unknown>
  wording: DeepLinkWording
}) {
  if (outcome.kind === 'ready') return null

  if (outcome.kind === 'resolving') {
    return <p className="text-muted-foreground text-sm">Loading {wording.noun}…</p>
  }
...
/**
 * The record is readable, just not in the organization currently open.
 *
 * Security Rules already allowed the read, which is the only thing that decides
 * whether this person may see it — so this is browser state, not authorization.
 *
 ...
 * only thing shown before they choose; nothing about the record appears until
 * they have.
 *
 * When none is open there is nothing to move and nobody to surprise, so the
 * record's organization is simply opened. This is the ordinary end of a scanned
...
function SwitchOrganization({
  ...
  if (!hasActiveOrganization) {
    return <p className="text-muted-foreground text-sm">Opening {name ?? wording.noun}…</p>
  }

  return (
    <Card className="mx-auto max-w-lg">
      <CardHeader>
        <CardTitle className="text-base">{wording.belongsTo}</CardTitle>
        <CardDescription>
